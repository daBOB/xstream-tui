...
| Browsing works but every download fails (`404`, or `456`/`511` on live) | The portal serves `player_api.php` to anyone but refuses media to VPN/datacenter IPs. Confirm by comparing one stream URL through the tunnel against the same URL from your own connection: a permitted IP answers `302` with a CDN redirect, a blocked one `404`. No exit country fixes this — it needs a provider that allows VPN access. |
