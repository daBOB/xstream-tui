...
func (m *EpisodesModel) View() string {
	...
	title := style.TitleStyle.Render("📺 " + m.season.Name)
	b.WriteString(title)
	b.WriteString("\n")

	// Season info
	info := style.ItemDimStyle.Render(fmt.Sprintf("%d episodes", len(m.episodes)))
	b.WriteString(info)
	b.WriteString("\n")
	...
	// Episodes header
	b.WriteString(style.SubtitleStyle.Render("Episodes"))
	b.WriteString("\n")
